...
/**
 * Subscriber que crea automáticamente una PREFACTURA (PRE) en Contifico
 * cuando se coloca un pedido (order.placed = pago autorizado).
 * La FACTURA (FAC) se crea en el subscriber de payment.captured.
 */
export default async function contificoOrderPlacedHandler({
    ...
    // Verificar que no exista ya una prefactura para esta orden
    const existing = await service.listContificoEntityMaps({
        entity_type: "invoice",
        medusa_id: orderId,
    })
    const activePRE = existing.filter(
        (e: any) => (e.metadata as any)?.estado !== "A" && (e.metadata as any)?.tipo_documento === "PRE"
    )
    if (activePRE.length > 0) {
        console.log(`[Contifico] Orden ${orderId} ya tiene prefactura, saltando`)
        return
    }
    ...
    try {
        ...
        // PRE = sin cobros, estado Pendiente
        ...
        const docPayload: ContificoDocumentoCreate = {
            ...
            estado: "P",
            electronico: "0", // PRE no pasa por SRI
            reserva_relacionada: null,
            referencia,
            descripcion: isTestMode
                ? `Auto-prefactura prueba Medusa - ${new Date().toLocaleString()}`
        ...
        console.log(`[Contifico] Auto-prefactura creada: ${docCreated.id} ref=${referencia}`)

    } catch (err) {
        console.error(`[Contifico] Error auto-prefacturando orden ${orderId}:`, err)
